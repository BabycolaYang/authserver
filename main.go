...
const (
	...
	defaultRedirectURL		 = "https://aiml-test.quantumcloudai.com/account/login.html"
)
...
func main() {
	...
	redirectURL := getEnvOrDefault("REDIRECT_URL",defaultRedirectURL)
	...
	router.HandleFunc("/login", s.callbackOption).Methods(http.MethodOptions)
	router.HandleFunc("/logout", s.callbackOption).Methods(http.MethodOptions)
